...
export function useCreateBotForm() {
        ...
        const loadState = async () => {
            try {
                if (isEditMode && characterId) {
                    const savedEdit =
                        await storage.getEditBotState<BotFormState>();
                    if (cancelled) return;
                    if (
                        savedEdit &&
                        savedEdit.editCharacterId === characterId
                    ) {
                        setForm(savedEdit);
                    } else {
                    ...
                    const saved =
                        await storage.getCreateBotState<BotFormState>();
                    if (cancelled) return;
                    if (saved) {
                        setForm(saved);
                    }
    ...
    const toggleTag = useCallback((tagId: number) => {
        setForm((f) => {
            ...
            if (
                !alreadySelected &&
                f.tag_ids.length + f.custom_tags.length >= 10
            ) {
                return f;
            }
