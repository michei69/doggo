...
import SwipeCard, { type SwipeDirection } from "./characterSearch/SwipeCard";
...
export default function SwipeDiscoverScreen() {
  ...
  const route =
    useRoute<RouteProp<CharactersStackParamList, "SwipeDiscover">>();
  const { hiddenIds, handleToggleHidden } = useHiddenCharacters();
  const { deck, loading, refreshing, error, refresh, loadMore } = useSwipeDeck(
    route.params,
    hiddenIds,
  );
  ...
  const caughtUp = !loading && deck.length > 0 && deckIndex >= deck.length;
